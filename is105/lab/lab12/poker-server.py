...
server = socket.socket(socket.AF_INET, socket.SOCK_STREAM) #AF_INET representerer adresse og protokollfamilie brukt som første argument for socket(). Andre argument, SOCK_STREAM representerer socket typen.
server.setblocking(0) #Blocking mode er satt til 0. Hvis recv() ikke finner data, eller send() ikke kan kvitte seg med data feilfritt, vil ein feileksepsjon bli reist. Om argumentet er satt til 1 vil prosessen blokkes til vi kan fortsette.

# Binde socketen på lokalmaskinen til porten 10000
server_address = ('localhost', 10000) #Adressen som serveren lytter til
print >>sys.stderr, 'starter socket på %s og port %s' % server_address #filnavn starter på socket x og port y
server.bind(server_address) #binder socket til ønsket adresse

# Høre / vente på innkommende forbindelser
server.listen(5) #Lytter etter forbindelser laget av socketen. Argumentet spesifiserer maksimum antall forbindelser i kø.
...
# tre ventende lister med interger eller objekter
...
while inputs: #liste inputs. Inneholder variabel server (socket.socket(socket.AF_INET, socket.SOCK_STREAM))
	# Vent inntil minst en av socketene er klar for prosessering
	print >>sys.stderr, '\nventer på neste hendelse'
	readable, writable, exceptional = select.select(inputs, outputs, inputs)
#readable, writable, exeptional er nå tre nye lister
...
# (3) socketene som er returnert gjennom exceptional har hatt en feil (definisjon av uttak er platformavhengig)
	...
	for key in message_queues.keys():
		print >>sys.stderr, 'spiller %s er meldt seg for spill' % str(key.getpeername())#getpeername returnerer adressen socketen er tilkoblet. I en for loop vil en melding printes hver gang noen kobler seg til spillet
...
# Behandler inputs her
	for s in readable: #sjekker readable lista
		if s is server: #hvis den er fra variabel server (socket.AF_INET, socket.SOCK_STREAM)
			# En lesbar (readable) server socket er klar til å akseptere forbindelser
			connection, client_address = s.accept() #Tilkobling aksepteres
			#s.accept tar to argumenter(conn, adress) Conn er et nytt socketobjekt brukbar til å sende og motta data på forbindelsen
			#adress er tilknyttet til socketen
			#Vi plasserer conn informasjonen i en lokal variabel, connection og gjør det samme for adresse i client_adress
			print >>sys.stderr, 'en ny forbindelse fra', client_address #Printer tilkoblet adresse
			connection.setblocking(0)# slår på non-blocking mode
			inputs.append(connection)#legger forbindelsen i inputs lista
...
#     Data leses med recv(), så blir plassert i en kø, slik at den kan bli sendt gjennom socketen tilbake til klienten
			...
			data = s.recv(1024) #mottar data fra socketen med max størrelse på 1024bytes. Return verdien er (String, adress)
					# srting representerer data motatt og adressen fra socketen som sender dataen
			if data:
				# En lesbar klient-socket som har data
				print >>sys.stderr, 'mottok "%s" fra %s' % (data, s.getpeername()) #print melding
